...
import earthTexture from "../Sub-Components/earthtexture.webp";
...
const CameraController = ({ distance }) => {
  ...
  useEffect(() => {
    if (distance) {
      ...
      camera.fov = 60
      // Keep the camera at a fixed Z position but adjust the zoom
...
const AsteroidSystem = ({ currentAsteroid, distance }) => {
  const asteroidPosition = [100, 0, 0]; // Fixed position for the asteroid on the right

  // Debug: Log distance and current Earth scale
  ...
  return (
    <>
      ...
      <Earth scale={earthScale} /> {/* Adjust the scale dynamically */}
      {currentAsteroid && (
        <Asteroid
...
export const Asteroids = () => {
  ...
  const currentAsteroid = asteroidData.length > 0 ? asteroidData[currentIndex] : null;
  ...
  const closeApproachDate = new Date(currentAsteroid.close_approach_data[0].close_approach_date_full);
  const timeDiffSeconds = (closeApproachDate - currentDate) / 1000; // Convert milliseconds to seconds

  // Calculate distance traveled using relative_velocity (in km/s) and the time difference
  const velocityKms = parseFloat(currentAsteroid.close_approach_data[0].relative_velocity.kilometers_per_second);
  const distanceTraveled = calculateDistanceTraveled(velocityKms, timeDiffSeconds);

  // Use the miss_distance.kilometers if we're past the approach date
  const distance = timeDiffSeconds > 0
    ? distanceTraveled
    : parseFloat(currentAsteroid.close_approach_data[0].miss_distance.kilometers);
    ...
    <PageLayout/>
      <div className="canvasScreen">
        <Canvas color="black" width={800} height={600}>
          ...
          <AsteroidSystem currentAsteroid={currentAsteroid} distance={distance} />
        </Canvas>
      </div>
      ...
      {currentAsteroid && (
        <AsteroidCard
          name={currentAsteroid.name}
          dia={currentAsteroid.estimated_diameter.kilometers.estimated_diameter_max.toFixed(
            2
          )}
