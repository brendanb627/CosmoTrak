      ...
      <FormControlLabel
        control={
          <Checkbox
            defaultChecked
            checked={showDwarfs}
            onChange={(e) => setShowDwarfs(e.target.checked)}
          />
        }
        label="Dwarfs"
      />
...
const SolarSytem = ({
  ...
  const [PlutoCoordinates, setPlutoCoordinates] = useState([]);
  const [CeresCoordinates, setCeresCoordinates] = useState([]);
  const [ErisCoordinates, setErisCoordinates] = useState([]);
  ...
  useEffect(() => {
    const fetchPlanetData = async () => {
      ...
      await fetchPlutoCoordinates();
      await fetchMakemakeCoordinates();
...
console.log(showDwarfs)
  return (
    <>
      ...
      {NeptuneCoordinates.x && showPlanets && (
        <PlanetLabel
      ...
      {PlutoCoordinates.x && showPlanets && showDwarfs && (
        <PlanetLabel
          position={PlutoCoordinates}
          scale={scale}
          language={language}
          color={"rgb(90, 90, 90)"}
          name={"Pluto"}
